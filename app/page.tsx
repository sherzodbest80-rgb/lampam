...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* HERO */}
      <section className="bg-gradient-to-br from-lampam-blue to-lampam-navy text-white px-5 py-20 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 bg-lampam-green/20 text-lampam-green-light px-4 py-2 rounded-full text-xs font-bold tracking-wider mb-7 border border-lampam-green/40 backdrop-blur-sm opacity-0 translate-y-8 transition-all duration-700" style={{ animation: "fadeInUp 0.8s 0.1s ease-out forwards" }}>
            <span className="w-2 h-2 bg-lampam-green-light rounded-full animate-pulse-dot" />
            🇹🇭 THAILAND ASLI MAHSULOTLAR
          </div>

          <h1 className="font-display text-4xl md:text-5xl font-extrabold leading-tight mb-5 tracking-tight opacity-0 translate-y-8" style={{ animation: "fadeInUp 0.8s 0.2s ease-out forwards" }}>
          ...
          <p className="text-lg opacity-90 mb-9 max-w-xl mx-auto opacity-0 translate-y-8" style={{ animation: "fadeInUp 0.8s 0.3s ease-out forwards" }}>
            Original LAMPAM mahsulotlari — xo'rozingizning kuchi, salomatligi va chidamliligi uchun professional yechim
          </p>

          <div className="flex flex-wrap justify-center gap-3 mb-10 opacity-0 translate-y-8" style={{ animation: "fadeInUp 0.8s 0.4s ease-out forwards" }}>
            {["Original Thailand", "BEPUL yetkazib berish", "Professional maslahat"].map((feat) => (
              <div key={feat} className="flex items-center gap-2 bg-white/10 px-5 py-2.5 rounded-full text-sm font-semibold backdrop-blur-sm border border-white/20 hover:bg-white/15 hover:-translate-y-0.5 transition-all">
                <span className="text-lampam-green-light font-extrabold">✓</span> {feat}
              </div>
            ))}
          </div>

          <Link href="#products" className="inline-flex items-center gap-2.5 bg-white text-lampam-navy px-10 py-4 rounded-full font-bold text-base no-underline shadow-2xl hover:-translate-y-1 hover:shadow-3xl transition-all opacity-0 translate-y-8" style={{ animation: "fadeInUp 0.8s 0.5s ease-out forwards" }}>
      ...
      <section className="px-5 py-20 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {categories.map((cat, i) => (
            <div key={i} className="observe-fade opacity-0 translate-y-8 transition-all duration-700 bg-white rounded-3xl p-7 border border-slate-200 cursor-pointer hover:-translate-y-2 hover:shadow-2xl transition-all relative overflow-hidden" style={{ ['--accent' as string]: cat.color, ['--accent-bg' as string]: cat.bg }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="observe-fade opacity-0 translate-y-8 transition-all duration-700 bg-white p-8 rounded-3xl border border-slate-200 text-center hover:-translate-y-2 hover:shadow-2xl">
                <div className="w-18 h-18 mx-auto mb-4 bg-gradient-to-br from-lampam-blue to-lampam-navy rounded-2xl flex items-center justify-center text-3xl shadow-lg shadow-lampam-blue/25 w-16 h-16">
                  {f.icon}
                </div>
